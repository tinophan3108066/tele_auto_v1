...
class Tomarket {
    ...
    async login(data) {
        ...
        try {
            ...
            if (res.data.status !== 200) {
                this.log(colors.red(`Login không thành công! Mã trạng thái: ${res.data.status}`));
                return null;
            }
    ...
    async countdown(t) {
        for (let i = t; i > 0; i--) {
            ...
            process.stdout.write(colors.white(`[ WATTING ] Cần chờ ${hours}:${minutes}:${seconds}     \r`));
    ...
    async main() {
        ...
        while (true) {
            ...
            for (let i = 0; i < datas.length; i++) {
                try {
                    const result = await this.processAccount(datas[i], doQuest);
                    if (result !== null) {
                        listCountdown.push(result);
                    }
                } catch (error) {
                    console.error(colors.red(`Error account: ${error.message}`));
                    continue;
                }
 ...
 extractUserData(queryId) {
    ...
    async processAccount(data, hoinhiemvu) {
        const user = this.extractUserData(data)
        const id = user.userId;
        const username = user.username;
        
        console.log(`========== ${colors.green(username.green)} ==========`);
