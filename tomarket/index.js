...
class Tomarket {
  ...
  async startFarming() {
    ...
    const formatEndFarming = DateTime.fromMillis(endFarming)
      .toISO()
      .split(".")[0];
  ...
  async dailyClaim() {
    ...
    this.log(
      colors.green("Điểm danh hàng ngày thành công, phần thưởng: ") +
        colors.white(poin)
    );
    return true;
  }
  ...
  async getBalance() {
    ...
    while (true) {
      ...
      this.log(
        colors.yellow("Thời gian hoàn thành farming : ") +
          colors.white(formatEndFarming)
      );
      if (this.playGame) {
  ...
  save(id, token) {
    const tokens = JSON.parse(fs.readFileSync(rPath("token.json"), "utf8") || "{}");
    tokens[id] = token;
  ...
  async main() {
    ...
    const dataFile = args.data || rPath("data.txt");
